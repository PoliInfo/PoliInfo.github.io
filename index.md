...
Classification taskでは、ある政策についての意見（推進すべき、廃止すべき等）を含む議会会議録中の「発言A」と、それと同じ政策について述べられた「発言B」（ただし意見を含んでいるとは限らない）が与えられる。
参加者は、発言Bが発言Aの意見に対して「賛成」、「反対」、「別観点の意見」、「意見なしor別トピック」のどの関係であるかに分類する。

フェイクニュースなどを鵜呑みにしないためには批判的思考（critical thinking）をもつことが重要であり、そのためには、世の中にどのような意見が存在し、それらが互いにどのような関係にあるかを俯瞰できることが必要である。
本タスクは、そのための要素技術と位置付けられる。
Web上の意見を俯瞰する取り組みとしてWISDOMや言論マップなどが存在しているが、これらはWeb上で個別に述べられた意見を集約・俯瞰するのに対し、本タスクでは議会での質疑や答弁といった対話の中での意見を対象とする点に特徴がある。
また、本タスクの分類が政策に対する意見の分類として十分であるかについても議論したい。

* Input: 同じキーワードを含む、議会会議録中の２つの「発言」

* Output: 「賛成」、「反対」、「別観点の意見」、「意見なしor別トピック」への分類
