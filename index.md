...
Classification taskでは、ある政策（〇〇を推進すべき、廃止すべき等）と、その政策そ示すキーワードを含む議会会議録中の「発言」が与えられる。
参加者は、政策が実現した際のメリットやデメリットが発言に含まれているか判断し、「メリットを含む」、「デメリットを含む」、「両方を含む」、「どちらも含まない」のどの関係であるかに分類する。

フェイクニュースなどを鵜呑みにしないためには批判的思考（critical thinking）をもつことが重要であり、そのためには、メリットとデメリットを網羅的に俯瞰できることが必要である。
本タスクは、そのための要素技術と位置付けられる。
Web上の意見を俯瞰する取り組みとしてWISDOMや言論マップなどが存在しているが、これらはWeb上で個別に述べられた意見を集約・俯瞰するのに対し、本タスクでは政策におけるメリットとデメリットを対象とする点に特徴がある。
また、本タスクの分類が批判的思考を支援するのに十分であるかについても議論したい。

* Input: 政策を示すキーワードを含む、議会会議録中の「発言」

* Output: 「メリットを含む」、「デメリットを含む」、「両方を含む」、「どちらも含まない」への分類
